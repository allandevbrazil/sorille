...
type ColorTheme = "aurora" | "ember" | "ocean" | "gold";
...
const THEMES: Record<ColorTheme, ThemeConfig> = {
  gold: {
    // Paleta da marca Sorille (DESIGN.md): fundo preto, partículas em tom dourado.
    hueStart: 38,
    hueRange: 20,
    saturation: 70,
    lightness: 55,
...
export default function FlowField({
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      // Fallback estático: um único frame com partículas paradas, sem loop de animação.
      for (const p of particles) {
        ctx.beginPath();
        ctx.arc(p.x, p.y, 1.3, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(${p.hue}, ${cfg.saturation}%, ${cfg.lightness}%, 0.5)`;
        ctx.fill();
      }
    } else {
      render();
