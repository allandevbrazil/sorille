var rpi = rpi || {};
rpi.Time = {
    getTime: function(a, e, f) {
        return "chat" == f ? this.getChatTime(a, e || "en") : f ? this.getFormatTime(a, f, e || "en") : this.getDefaultTime(a, e || "en")
    },
    getChatTime: function(a, e) {
        var f = ((new Date).getTime() - a) / 1E3 / 60 / 60,
            l = f / 24;
        return 24 > f ? this.getFormatTime(a, "HH:mm", e) : 365 > l ? this.getFormatTime(a, "dd.MM HH:mm", e) : this.getFormatTime(a, "yyyy.MM.dd HH:mm", e)
    },
    getDefaultTime: function(a, e) {
        return this.getTimeAgo(a, e)
    },
    getTimeAgo: function(a, e) {
        a = ((new Date).getTime() - a) / 1E3;
        var f = a / 60,
            l = f / 60,
            d = l / 24,
            m = d / 365;
        e = rpi.Time.Messages[e] ?
            e : "en";
    ...
    getTime12: function(a, e) {
        a = new Date(a);
        return (a.getHours() % 12 ? a.getHours() % 12 : 12) + ":" + a.getMinutes() + (12 <= a.getHours() ? " PM" :
            " AM")
    },
    getFormatTime: function(a, e, f) {
        var l = new Date(a),
            d = {
                SS: l.getMilliseconds(),
                ss: l.getSeconds(),
                mm: l.getMinutes(),
                HH: l.getHours(),
                hh: (l.getHours() % 12 ? l.getHours() % 12 : 12) + (12 <= l.getHours() ? "PM" : "AM"),
                dd: l.getDate(),
                MM: l.getMonth() + 1,
                yyyy: l.getFullYear(),
                yy: String(l.getFullYear()).toString().substr(2, 2),
                ago: this.getTimeAgo(a, f),
                12: this.getTime12(a, f)
            };
        return e.replace(/(SS|ss|mm|HH|hh|DD|dd|MM|yyyy|yy|ago|12)/g, function(m, h) {
            m = d[h];
            return 10 > m ? "0" + m : m
        })
    },
    declineNum: function(a, e, f, l) {
        return a + " " +
            this.declineMsg(a, e, f, l)
    },
    declineMsg: function(a, e, f, l, d) {
        var m = a % 10;
        return 1 == m && (1 == a || 20 < a) ? e : 1 < m && 5 > m && (20 < a || 10 > a) ? f : a ? l : d
    }
};
rpi.Time.Messages = {
    ru: {
...
rpi.Utils = {
    ...
    getParent: function(a, e) {
        e = e || "rplg";
        if (0 > a.className.split(" ").indexOf(e))
            for (;
                (a = a.parentElement) && 0 > a.className.split(" ").indexOf(e););
        return a
    },
    lang: function() {
        var a = navigator;
    ...
    ajax: function(a, e, f, l) {
        const d = new XMLHttpRequest;
        d.open(e, a, !0);
        d.setRequestHeader("Content-Type", "application/json");
        d.onreadystatechange = function() {
            if ((d.readyState === XMLHttpRequest.DONE || d.readyState === d.DONE) && 200 === d.status) {
                const m = JSON.parse(d.responseText);
                "function" === typeof l ? l(m) : "function" === typeof f && f(m)
            }
        };
        d.send(f && "function" !== typeof f ? JSON.stringify(f) : null)
    },
    anchor: function(a, e, f, l, d) {
        const m = [];
        f.open_link &&
            m.push("noopener");
        f.nofollow_link && m.push("nofollow");
        return '<a href="' + a + '"' + (l ? ' class="' + l + '"' : "") + (f.open_link ? ' target="_blank"' : "") + (m.length ? ' rel="' + m.join(" ") + '"' : "") + (d ? ' aria-label="' + d + '"' : "") + ">" + e + "</a>"
    },
    rm: function(a) {
...
rpi = rpi || {};
rpi.Column = function(a, e, f) {
    const l = a.getAttribute("data-id"),
        d = new RegExp(f.col + "-[xsml]+"),
        m = new RegExp('$|(\\[data-id="' + l + '"\\]\\s+.' + f.card + "\\s*{\\s*--col:\\s*\\d+\\s*!important\\s*;?(\\s*--gap:\\s*\\d+\\s*!important;?)?})"),
        h = a.getElementsByClassName(f.cnt)[0];
    var n = null;
    return n = {
        init: function(g, p) {
            rpi.Utils.isVisible(h) ? (g && g(), n.resize(), window.addEventListener("resize", n.resize), p && p(), rpi.Instances = rpi.Instances || [], rpi.Instances.push(n)) : setTimeout(n.init, 300)
        },
        resize: function() {
            let g =
                n.getSize();
            h.className = h.className.replace(d, f.col + "-" + g);
            if (e.breakpoints) {
                let p = h.offsetWidth,
                    c = e.breakpoints.split(",");
                c.sort(n.brsort);
        ...
        getSize: function() {
            let g = h.offsetWidth;
            return 510 > g ? "xs" : 750 > g ? "x" : 1100 > g ? "s" : 1450 > g ? "m" : 1800 > g ? "l" : "xl"
        },
        setCol: function(g) {
            const p = this.getStyle();
            p.innerHTML = p.innerHTML.replace(m, (l ? '[data-id="' + l + '"] ' : "") + "." + f.card + "{--col:" + g + "!important;--gap:" + (g -
                1) + "!important}")
        },
        getCol: function() {
            let g = this.getStyle().innerHTML.match(/--col:\s*(\d+)/);
            return g && 1 < g.length ? g[1] : !1
        },
        getStyle: function() {
            let g = document.getElementById("rpi-style");
            g || (g = document.createElement("style"), g.id = "rpi-style", document.head.appendChild(g));
            return g
        },
        setBreakpoints: function(g) {
            e.breakpoints = g
        },
        brsort: function(g, p) {
            return parseInt(g.split(":")[0]) > parseInt(p.split(":")[0]) ? 1 : -1
        },
        clear: function() {
...
rpi = rpi || {};
rpi.Common = function(a, e, f) {
    var l = null;
    return l = {
        init: function() {
            var d = a.getElementsByClassName(f.time);
            let m = a.getElementsByClassName(f.text);
            for (var h = 0; h < d.length; h++) d[h].innerHTML = l.time(d[h].getAttribute("data-time"));
            for (d = 0; d < m.length; d++)(h = m[d]) && h.innerHTML && (h.innerHTML = l.trimtext(h.innerHTML), h = h.getElementsByClassName(f.readmore)[0]) && (h.onclick = l.opentext)
        },
        initOnce: function(d) {
            var m = d.getElementsByClassName(f.time)[0];
            d = d.getElementsByClassName(f.text)[0];
            m && (m.innerHTML = l.time(m.getAttribute("data-time")));
            d && d.innerHTML && (d.innerHTML = l.trimtext(d.innerHTML), m = d.getElementsByClassName(f.readmore)[0]) && (m.onclick = l.opentext)
        },
        time: function(d) {
            return e.time_format ? d : rpi.Time.getTimeAgo(1E3 * parseInt(d), rpi.Utils.lang())
        },
        trimtext: function(d, m) {
            var h = parseInt(e.text_size);
            if (d && h && d.length > h) {
                d = Array.from(d);
                let n = d.slice(0, h).join("").indexOf(" ") + 1;
                if (1 > n || h - n > h / 2) n = d.length > h ? h : d.length;
                let g = h = "";
                0 < n && (h = d.slice(0, n - 1).join(""), g = d.slice(n - 1).join(""));
                return h + (!m && g ? '<span class="rpi-s">... </span><input type="hidden" value="' +
                    encodeURIComponent(g) + '"></input><span class="' + f.readmore + '">' + rpi.Utils.__("read more", e.trans) + "</span>" : "")
            }
            return d
        },
        opentext: function() {
            let d = this.parentNode,
                ...
                h = this.previousSibling.previousSibling,
...
rpi.Media = function(a, e, {
    ...
    return m = {
        init: function() {
            d = a.getElementsByClassName("rpi-thumb");
            for (let h = 0; h < d.length; h++) d[h].setAttribute("data-idx", h),
                function(n, g) {
                    n.onclick = function() {
                        m.show(n, g);
                        return !1
        ...
        clicks: function(h) {
            h = h.getElementsByClassName("rpi-thumb");
            for (let n = 0; n < h.length; n++)(function(g, p) {
                g.onclick = function() {
                    let c = parseInt(g.getAttribute("data-idx"));
                    m.show(d[c], c);
                    return !1
                }
            })(h[n], n)
        ...
        show: function(h, n) {
            if (!(0 > n || n >=
                    d.length)) {
                var g = document.querySelector(".rpi-lightbox"),
                    p = h.src.replace(/=s[0-9]{1,3}/g, "=s500");
                h = rpi.Utils.getParent(h, l).outerHTML;
                if (g) {
                    let c = g.querySelector(".rpi-lightbox-media img"),
                        r = g.querySelector(".rpi-lightbox-review"),
                        t = g.querySelector(".rpi-lightbox-prev");
                    next = g.querySelector(".rpi-lightbox-next");
                    c.src = p;
                    r.innerHTML !== h && (r.innerHTML = h, m.clicks(r));
                    t.onclick = function(w) {
                        m.show(d[n - 1], n - 1)
                    };
                    next.onclick = function(w) {
                        m.show(d[n + 1], n + 1)
                    }
                } else g = document.createElement("div"), g.className =
                    ...
                    function() {
                        ...
                        g == c.target && (g.style.display = "none")
                    }, h.onclick = function(c) {
                        m.show(d[n - 1], n - 1)
                    }, next.onclick = function(c) {
                        m.show(d[n + 1], n + 1)
                    }, m.clicks(g), document.body.appendChild(g);
...
rpi = rpi || {};
rpi.Slider = function(a, e, f, l) {
    a.getAttribute("data-id");
    const d = a.getElementsByClassName(f.cnt)[0],
        m = a.getElementsByClassName(f.content)[0],
        h = m ? m.getElementsByClassName(f.cards)[0] : null,
        n = m ? m.getElementsByClassName(f.dotsWrap)[0] : null,
        g = m ? m.getElementsByClassName(f.dots)[0] : null,
        p = h ? parseInt(h.getAttribute("data-count")) : 0;
    var c = null,
        r = m ? m.getElementsByClassName(f.card) : null,
        t = "",
        w = "",
        z = null,
        x = null,
    ...
    return c = {
        init: function(b, k) {
            rpi.Utils.isVisible(d) ? (b && b(), c.resize(),
                c.actions(), r.length && c.swipeAutoStart(), k && k(), rpi.Instances = rpi.Instances || [], rpi.Instances.push(c)) : setTimeout(c.init, 300)
        },
        resize: function(b) {
            let k = l.column.resize(),
                q = l.column.getCol();
            b && h.scrollLeft != b * c.reviewWidth() && h.scrollTo(r[b].offsetLeft, 0);
            !r.length || t == k && w == q || (c.hasDots() && (c.dotsInit(), c.dotSwipe(b, !0)), t = k, w = q)
        },
        actions: function() {
            e.mousestop && c.addMouseEvents();
            window.addEventListener("resize", c.resizeListener);
            h && (h.addEventListener("scroll", c.scrollListener, !1), e.wheelscroll &&
                m.addEventListener("wheel", c.wheelListener, !1));
            var b = d.getElementsByClassName(f.btnPrev)[0];
            b && (b.onclick = function(k) {
                c.btnClick(-1)
            });
            if (b = d.getElementsByClassName(f.btnNext)[0]) b.onclick = function(k) {
                c.btnClick(1)
            }
        },
        resizeListener: function() {
            var b = D;
            clearTimeout(z);
            z = setTimeout(c.resize, 150, b)
        },
        scrollListener: function() {
            clearTimeout(x);
            clearTimeout(v);
            v = setTimeout(c.scrollEnd, 150)
        },
        wheelListener: function(b) {
            var k = b.target;
            if ((k = -1 < k.className.indexOf(f.text) ? k : -1 < k.parentNode.className.indexOf(f.text) ?
                    k.parentNode : null) && k.scrollHeight > k.clientHeight) return !0;
            b.preventDefault();
            C++;
            clearTimeout(A);
            A = setTimeout(c.wheelEnd, 150, b)
        },
        addMouseEvents: function() {
            d.addEventListener("mouseover", c.mouseOver, !1);
            d.addEventListener("mouseleave", c.mouseLeave, !1)
        },
        delMouseEvents: function() {
            d.removeEventListener("mouseover", c.mouseOver);
            d.removeEventListener("mouseleave", c.mouseLeave)
        },
        mouseOver: function() {
            B = 1;
            c.swipeAutoStop()
        ...
        wheelEnd: function(b) {
            c.swipeManual(Math.sign(b.wheelDelta) * C * c.swipeStep());
            C = 0
        },
        scrollEnd: function() {
            D = c.reviewsIdx();
            y ? y = !1 : c.loadNextReviews();
            (!e.mousestop || B) && e.mousestop || (!e.clickstop || y) && e.clickstop || c.swipeAutoStart();
            c.hasDots() && c.dotSwipe(void 0, !0)
        },
        loadNextReviews: function(b) {
            b = b ? c.reviewsIdx() + parseInt(b) : c.hasDots() ? (b = g.getElementsByClassName("active")[0]) ? parseInt(b.getAttribute("data-index")) * c.swipePerDot() : c.reviewsIdx() : c.reviewsIdx();
            b = c.getAjaxSize(b);
            0 <
                b && l.view.loadNextReviews(b)
        },
        getAjaxSize: function(b) {
            let k = 0;
            const q = parseInt(h.getAttribute("data-offset")),
                u = parseInt(e.pagination);
            if (p > q) {
                let E = b - q;
                Math.abs(E) < 3 * c.swipePerDot() ? k = u : E && (k = Math.ceil(b / u) * u - q)
            }
            b = q + k - p;
            return 0 < b ? k - b : k
        },
        dotsInit: function() {
            if (g) {
                var b = Math.round(p / c.swipePerDot());
                g.innerHTML = "";
                for (let k = 0; k < b; k++) {
                    let q = document.createElement("div");
                    q.className = f.dot;
                    q.setAttribute("data-index", k);
                    q.setAttribute("title", k);
                    q.onclick = c.dotClick;
                    g.appendChild(q)
                }
                c.dotsPadding()
            }
        ...
        dotClick: function() {
            ...
            var k = g.getElementsByClassName("active")[0];
            k = parseInt(k.getAttribute("data-index"));
            c.swipeManual(Math.abs(b - k) * c.swipePerDot() * Math.sign(b - k))
        },
        dotsPadding: function() {
            let b = n.getBoundingClientRect().height;
            d.style.paddingBottom = b + "px"
        },
        dotSwipe: function(b, k) {
            b = Math.round((void 0 !== b ? b : c.reviewsIdx()) / c.swipePerDot());
            b = 0 > b ? 0 : b >= g.childNodes.length ? g.childNodes.length - 1 : b;
            b = g.querySelector("." + f.dot + '[data-index="' + b + '"]');
            let q =
                g.getElementsByClassName("active")[0];
            c.dotActivate(q, b);
            c.dotScroll(b, k)
        },
        dotScroll: function(b, k) {
            let q = Math.round(g.scrollWidth / g.childNodes.length),
                u = Math.floor(Math.round(g.offsetWidth / q) / 2);
            k ? g.scrollTo(b.offsetLeft - u * q, 0) : g.scrollTo({
                left: b.offsetLeft - u * q,
                behavior: "smooth"
            })
        },
        dotActivate: function(b, k) {
            b && (b.classList.remove("active"), b.classList.remove("s1"), b.previousSibling && (b.previousSibling.classList.remove("s2"), b.previousSibling.previousSibling && b.previousSibling.previousSibling.classList.remove("s3")),
                b.nextSibling && (b.nextSibling.classList.remove("s2"), b.nextSibling.nextSibling && b.nextSibling.nextSibling.classList.remove("s3")));
            k && (k.classList.add("active"), k.classList.add("s1"), k.previousSibling && (k.previousSibling.classList.add("s2"), k.previousSibling.previousSibling && k.previousSibling.previousSibling.classList.add("s3")), k.nextSibling && (k.nextSibling.classList.add("s2"), k.nextSibling.nextSibling && k.nextSibling.nextSibling.classList.add("s3")))
        },
        swipeManual: function(b) {
            y = !0;
            c.loadNextReviews(b);
            c.scroll(b);
            e.clickstop && (c.swipeAutoStop(), c.delMouseEvents())
        },
        swipeAuto: function() {
            if (c.isScrollEnd()) c.scroll(-(p - c.reviewsPerView()));
            else {
                let b = c.swipeStep() < c.reviewsAhead() ? c.swipeStep() : c.reviewsAhead();
                c.scroll(b)
            }
            c.swipeAutoStart()
        },
        scroll: function(b) {
            b = c.reviewsIdx() + parseInt(b);
            let k = rpi.Utils.isRTL() ? b + c.reviewsPerView() - 1 : b;
            k = 0 > k ? 0 : k >= r.length ? r.length - 1 : k; - 1 < k && k < r.length && (h.scrollTo({
                left: r[k].offsetLeft,
                behavior: "smooth"
            }), c.hasDots() && c.dotSwipe(b))
        },
        swipeAutoStart: function() {
            e.autoplay &&
                (x = setTimeout(c.swipeAuto, 1E3 * parseInt(e.speed)))
        },
        ...
        isScrollEnd: function() {
            var b = h.querySelector("." + f.card + ":last-child"),
                k = b.getBoundingClientRect();
            b = b.parentNode.getBoundingClientRect();
            return (2 > Math.abs(b.left - k.left) || b.left <= k.left) && k.left < b.right && (2 > Math.abs(b.right - k.right) || b.right >= k.right) && k.right > b.left
        },
        swipeStep: function() {
            return e.swipe_step || c.reviewsPerView()
        },
        swipePerBtn: function() {
            return e.swipe_per_btn ||
                c.reviewsPerView()
        },
        swipePerDot: function() {
            return e.swipe_per_dot || c.reviewsPerView()
        },
        reviewWidth: function() {
            return Math.round(h.scrollWidth / r.length)
        },
        reviewHeight: function() {
            return r[0].offsetHeight
        },
        reviewsPerView: function() {
            return Math.round(h.offsetWidth / c.reviewWidth())
        },
        reviewsIdx: function() {
            let b = rpi.Utils.isRTL() ? -h.scrollLeft : h.scrollLeft;
            return Math.round(b / c.reviewWidth())
        },
        reviewsAhead: function() {
            return r.length - (c.reviewsIdx() + c.reviewsPerView())
        },
        hasDots: function() {
            return g && !e.hide_dots &&
                0 < c.swipePerDot()
        },
        setBreakpoints: function(b) {
            l.column.setBreakpoints(b);
            c.resize()
        },
        clear: function() {
            clearTimeout(z);
            ...
            clearTimeout(v);
            clearTimeout(A);
            window.removeEventListener("resize", c.resizeListener);
            h.removeEventListener("scroll", c.scrollListener);
            m.removeEventListener("wheel", c.wheelListener)
        }
    }
};

function rplg_badge_init(a, e, f) {
    var l = a.querySelector(".wp-" + e + "-badge"),
        d = a.querySelector(".wp-" + e + "-form");
    l && d && (a = document.createElement("div"), a.className = f + " wpac", -1 < l.className.indexOf("-fixed") && a.appendChild(l), a.appendChild(d), document.body.appendChild(a), l.onclick = function() {
        d.style.display = "block"
    })
}

function rplg_next_reviews(a, e) {
    var f = this.parentNode,
        l = "." + a + "-review." + a + "-hide";
    reviews = f.querySelectorAll(l);
    for (var d = 0; d < e && d < reviews.length; d++) reviews[d] && (reviews[d].className = reviews[d].className.replace(a + "-hide", " "));
    reviews = f.querySelectorAll(l);
...
function grw_init(a, e) {
    ...
    if ("true" != a.getAttribute("data-exec")) {
        ...
        !a.getElementsByClassName("grw-review")[0] || "slider" != e && "grid" != e || (e = a.getElementsByClassName("grw-row")[0], e = JSON.parse(e.getAttribute("data-options")), f = rpi.Column(a,
            e, {
                cnt: "grw-row",
                col: "grw-row",
                card: "grw-review"
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let f = 0; f < a.length; f++) {
        var e = a[f];
        grw_init(e, e.getAttribute("data-layout"))
    }
});